...
class OrderStats {
    ...
    static List<Order> ordersForCardType(final Stream<Customer> customers, PaymentInfo.CardType cardType) {
        ...
        return customers.flatMap(orderElement ->orderElement.getOrders().stream())
                .filter(orderElement ->orderElement.getPaymentInfo().getCardType().equals(cardType))
    ...
    static Map<Integer, List<Order>> orderSizes(final Stream<Order> orders) {
        ...
        return orders.collect(
                Collectors.groupingBy(elem ->elem.getOrderItems().stream()
    ...
    static Boolean hasColorProduct(final Stream<Order> orders, final Product.Color color) {
        ...
        return orders.allMatch(element ->element.getOrderItems().stream()
    ...
    static Map<String, Long> cardsCountForCustomer(final Stream<Customer> customers) {
        ...
        return customers.collect(Collectors.toMap(element-> element.getEmail(),
                element ->element.getOrders().stream()
                        .map(orderList ->orderList.getPaymentInfo())
                        .distinct()
                        .map(payment->payment.getCardNumber())
                        .count()));
    }
...
//        customers.sorted().collect(Collectors.groupingBy(element -> element.getAddress().getCountry())).;
        return null ;
    }
